<template>
  <div>
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="relative overflow-hidden rounded-lg bg-dark-800 px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6">
        <dt>
          <div class="absolute rounded-md bg-primary-500 p-3">
            <PhoneIcon class="h-6 w-6 text-white" aria-hidden="true" />
          </div>
          <p class="ml-16 truncate text-sm font-medium text-gray-300">Status WhatsApp</p>
        </dt>
        <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
          <p class="text-2xl font-semibold text-white">Terhubung</p>
          <div class="absolute inset-x-0 bottom-0 bg-dark-300 px-4 py-4 sm:px-6">
            <div class="text-sm">
              <button class="btn-primary w-full" @click="scanQRCode">
                Scan QR Code
              </button>
            </div>
          </div>
      ...
      <div class="relative overflow-hidden rounded-lg bg-dark-800 px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6">
        <dt>
          ...
          <p class="ml-16 truncate text-sm font-medium text-gray-300">Pesan Masuk Hari Ini</p>
        </dt>
        <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
      ...
      <div class="relative overflow-hidden rounded-lg bg-dark-800 px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6">
        <dt>
          ...
          <p class="ml-16 truncate text-sm font-medium text-gray-300">Pesan Keluar Hari Ini</p>
        </dt>
        <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
      ...
      <div class="relative overflow-hidden rounded-lg bg-dark-800 px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6">
        <dt>
          ...
          <p class="ml-16 truncate text-sm font-medium text-gray-300">Pelanggan Aktif</p>
        </dt>
        <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
    ...
    <!-- Recent Messages -->
    <div class="mt-8">
      <div class="rounded-lg bg-dark-800 shadow">
        <div class="p-6">
          <h3 class="text-base font-semibold leading-6 text-white">Pesan Terbaru</h3>
          <div class="mt-4 flow-root">
            <div class="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
              <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                ...
                message.customer }}</td>
                      <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-300">{{ message.content }}</td>
                      <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-300">
                        <span
                          :class="[message.status === 'Terkirim' ? 'text-green-600 bg-green-900' : 'text-yellow-600 bg-yellow-900', 'inline-flex rounded-full px-2 text-xs font-semibold leading-5']">{{ message.status }}</span>
                      </td>
                      <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-300">{{ message.time }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
